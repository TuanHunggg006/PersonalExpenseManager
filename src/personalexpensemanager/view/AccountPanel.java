...
public class AccountPanel extends JPanel {
    ...
    private JPanel createDetailsPage(String walletName) {
        ...
        for (String date : grouped.keySet()) {
            ...
            for (Transaction t : grouped.get(date)) {
                ...
                item.add(lblIcon, BorderLayout.WEST); // Đặt icon ở phía Tây
                item.add(cat, BorderLayout.CENTER);  // Chữ hạng mục ở giữa
    ...
    // HÀM MỚI: LẤY ICON THEO HẠNG MỤC (Đã sửa logic đuôi file khớp với thư mục của bạn)
    private ImageIcon getIconForCategory(String category) {
        ...
        switch (category) {
            ...
            case "Lương": fileName = "icon_luong.png"; break; // Dùng đuôi png hoặc jpg tùy bạn đã sửa trong folder chưa
